...
function build_lib() {
    ...
    sudo $HOST-dlltool -d $MODULE.def -l $MINGW_ROOT/$HOST/lib/$LIBFILE
}
...
sudo make -f win32/Makefile.gcc install SHARED_MODE=1 \
...
ARGS="$PLAT DINPUT=n WINDRES=$HOST-windres STRIP=$HOST-strip NEW_DYNAREC=y"
